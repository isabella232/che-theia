...
export class CheSideCarFileSystemImpl implements CheSideCarFileSystem {
  ...
  async $rename(from: string, to: string, opts: FileOverwriteOptions): Promise<void> {
    ...
    try {
      ...
      if (error.code === 'EINVAL' || error.code === 'EBUSY' || error.code === 'ENAMETOOLONG') {
        error = new Error(`Unable to move '${basename(from)}' into '${basename(dirname(to))}' (${error.toString()}).`);
      }
  ...
  async $copy(from: string, to: string, opts: FileOverwriteOptions): Promise<void> {
    ...
    try {
      ...
      // to something the user can better understand
      if (error.code === 'EINVAL' || error.code === 'EBUSY' || error.code === 'ENAMETOOLONG') {
        error = new Error(`Unable to copy '${basename(from)}' into '${basename(dirname(to))}' (${error.toString()}).`);
      }
  ...
  private async validateTargetDeleted(
    ...
    const isSameResourceWithDifferentPathCase = from.toLowerCase() === to.toLowerCase();
    // }
    ...
    if (!isSameResourceWithDifferentPathCase && (await promisify(exists)(to))) {
      if (!overwrite) {
